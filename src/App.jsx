...
// ── AXIS AI center nav icon ───────────────────────────────────
const AxisAINavIcon = ({ active }) => (
  <div style={{
    width: 42, height: 42, borderRadius: "50%",
    background: active ? "linear-gradient(135deg,#6366F1,#8B5CF6)" : "#1A1A28",
    border: `2px solid ${active ? "transparent" : "#2A2A3F"}`,
...
// ── Tab config ────────────────────────────────────────────────
const TABS = [
  { id: "command",  label: "Command",  Icon: CommandIcon,  Page: Dashboard },
  { id: "study",    label: "Study",    Icon: StudyIcon,    Page: Study },
  { id: "axis-ai",  label: "AXIS AI",  Icon: null,         Page: AIChat, isCenter: true },
...
function PageWrapper({ children, tabId }) {
  return (
    <div key={tabId} style={{ animation: "fadeIn 0.22s ease both", minHeight: "100vh" }}>
      {children}
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0A0F", position: "relative" }}>
          ...
          return (
            <button key={id} onClick={() => setActiveTab(id)} style={{
              ...
              <span style={{
                ...
                fontWeight: 700, marginTop: isCenter ? 6 : 0
              }}>
                {label}
              </span>
            </button>
      ...
      <Toaster
        ...
        toastOptions={{
          style: {
            background: "#1A1A28", color: "#F1F5F9",
            border: "1px solid #2A2A3F", borderRadius: 12,
            fontSize: "0.875rem", fontFamily: "'DM Sans', sans-serif",
            fontWeight: 500, boxShadow: "0 8px 32px rgba(0,0,0,0.6)", maxWidth: 340,
          },
